...
const DeleteLevelModal = ({id}) => {
  const { handleApiResponse } = useApiToast()
  const [deleteApi] = useDeleteApiMutation()
  const [setLoading] = useLoading()
  const handelDeleteSubject = async () =>{
    setLoading(true)
   const response = await deleteApi({url : `/subject/${id}`, tag : "subject"})
   handleApiResponse("حذف مادة", response)
 }
   return (
    <>
                            ...
                            <div className="grid flex-1 gap-2">
                                 ...
                                 className=" bg-red-600 h-8 w-1/3 hover:bg-red-700" type="button"  >  
                                  ...
                                  <Button
                                   ...
                                   type="button" variant="secondary" className="w-1/3 h-8   ">
                                      إلغاء
                                  </Button>
