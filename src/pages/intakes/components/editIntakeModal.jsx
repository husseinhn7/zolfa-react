  ...
  const [setLoading] = useLoading()
     const {handleSubmit, handleChange, values, setFieldValue, errors, touched} = useFormik({
        ...
        onSubmit : async (values)=>{
          setLoading(true)
          const response = await updateApi({url:`/intake/${data.data._id}`, body:values, tag : "intake"})
              ...
              <div className="flex flex-col  w-full items-center gap-4  justify-between">
                ...
                <div className="flex flex-col w-full" dir='rtl'>
                    ...
                    <Select
                              dir="rtl"
                              name="level"
                              value={values.level} // Ensure this matches one of the options
                              onValueChange={(value) => setFieldValue("level", value)}
                            >
                              <SelectTrigger id="level">
                                <SelectValue placeholder="المستوي">
                                  {levels?.data?.data.find((level) => level._id === values.level)?.name || "المستوي"}
                                </SelectValue>
                              </SelectTrigger>
                              <SelectContent position="popper">
                                {levels && !levelsFetching
                                  ? levels.data.data.map((level) => (
                                      <SelectItem key={level._id} value={level._id}>
                                        {level.name}
                                      </SelectItem>
                                    ))
                                  : <SelectItem disabled>تحميل...</SelectItem>}
                              </SelectContent>
                            </Select>
                ...
                <div className="flex flex-col w-full">
                  ...
                  <Input value={values.description}  name="description" placeholder="وصف الدفعة" onChange={handleChange} />
                </div>
              </div>
